...
function Board(container, images, sounds) {
	this.images = images;
	this.sounds = sounds;
	this.pos = new Position(true);
	// let fen = "r*n*b*a*ka*b*n*r*/9/1c*5c*1/p*1p*1p*1p*1p*/9/9/P*1P*1P*1P*1P*/1C*5C*1/9/R*N*B*A*KA*B*N*R* w - - 0 1";
	...
	for (var sq = 0; sq < 256; sq++) {
		...
		style.left = SQ_X(sq);
		style.top = SQ_Y(sq);
		style.width = String(SQUARE_SIZE);
		style.height = String(SQUARE_SIZE);
		style.zIndex = 0;
...
Board.prototype.addMove = function (mv, computerMove) {
	console.log("addMove:", mv, "computerMove:", computerMove, move2Iccs(mv), this.pos.squares[SRC(mv)], "-->", this.pos.squares[DST(mv)])
...
Board.prototype.postAddMove = function (mv, computerMove) {
	...
	// 一方走完一步后，棋盘上会有两个标记，指示从哪走到了哪 src-->dst
...
Board.prototype.response = function () {
	...
	this.busy = true;
	setTimeout(function () {
		// let robotmv = board.search.searchMain(LIMIT_DEPTH, board.millis)
		let robotmv = this_.pos.randomMove();
		console.log("robotmv:", robotmv)
		this_.addMove(robotmv, true);
...
Board.prototype.clickSquare = function (sq_) {
	...
	if ((pc & SIDE_TAG(this.pos.sdPlayer)) != 0) {
		...
		console.log('fen:', this.pos.toDarkChessFen())
		console.log(`click:sqSelected: ${this.sqSelected}, sq;${sq}, 暗bugchess:${this.pos.bugchess_square[sq]} 暗:${PIECE_2_CHAR[pc]}, 明:${PIECE_2_CHAR[this.pos.DarkToReal(sq)]}`)
		this.drawSquare(sq, true);
		this.sqSelected = sq;
	} else if (this.sqSelected > 0) {
		console.log("addMove:", PIECE_2_CHAR[this.pos.squares[this.sqSelected]], this.sqSelected, "--->", sq, PIECE_2_CHAR[this.pos.DarkToReal(sq)])
...
Board.prototype.drawSquare = function (sq, selected) {
	...
	if(this.pos.isDark(sq)){
		img.src = this.images + PIECE_NAME_DARK[this.pos.DarkToReal(sq)];
	}else{
		img.src = this.images + PIECE_NAME[this.pos.squares[sq]] + ".gif";
	}
